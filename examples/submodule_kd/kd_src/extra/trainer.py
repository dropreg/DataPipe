...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, tec_model, criterion, quantizer=None):
        ...
        # lxb edit start: add tec model
        self._tec_model = tec_model
        # lxb edit end: add tec model

        if cfg.common.fp16:
            self._criterion = self._criterion.half()
            self._model = self._model.half()
            self._tec_model = self._tec_model.half()
        elif cfg.common.bf16:
            self._criterion = self._criterion.to(dtype=torch.bfloat16)
            self._model = self._model.to(dtype=torch.bfloat16)
            self._tec_model = self._tec_model.to(dtype=torch.bfloat16)
        if (
            ...
            self._tec_model = self._tec_model.to(device=self.device)
    ...
    @property
    def tec_model(self):
        if self._wrapped_tec_model is None:
            if self.use_distributed_wrapper:
                self._wrapped_tec_model = models.DistributedFairseqModel(
                    self.cfg.distributed_training,
                    self._tec_model,
                    process_group=self.data_parallel_process_group,
                    device=self.device,
                ...
                self._wrapped_tec_model = self._tec_model
        return self._wrapped_tec_model
    # lxb edit end:
    ...
    # lxb edit: add load tec ckpt
    def load_tec_checkpoint(self, filename):

        logger.info(f"Preparing to load teacher model checkpoint {filename}")
        is_distributed = self.data_parallel_world_size > 1
        bexists = PathManager.isfile(filename)
        if bexists:
            load_on_all_ranks = (
            ...
            # load model parameters
            try:
                self.tec_model.load_state_dict(state["model"], strict=True, model_cfg=self.cfg.model)
            except Exception:
                raise Exception(
                    "Cannot load model parameters from checkpoint {}; "
                    "please ensure that the architectures match.".format(filename)
            ...
            logger.info("No existing teacher model checkpoint found {}".format(filename))
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        self._set_seed()
        self.model.train()
        # lxb edit start: set tec model to eval mode 
        self.tec_model.eval()
        # lxb edit end
